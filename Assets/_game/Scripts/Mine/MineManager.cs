...
public class MineManager : MonoBehaviour
{
    ...
    public int GetLastOpenedMine() {
        int last = Items.Count - 1;
        for (int i = Items.Count-1; i >=0; i--) {
            if (Items[i].Stats.IsOpened)
            {
                last = i;
                break;
            }
    ...
    public float GetMineBoostPrice(int mineInd) {
        if (mineInd < 0 || mineInd >= Items.Count) return 0;
        return Items[mineInd].Stats.BoostCost;
    }
